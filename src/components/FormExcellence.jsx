...
export default function FormExcellence({ onBack }) {
  ...
  const capturePhoto = async () => {
    if (videoRef.current && canvasRef.current && isCameraActive) {
      ...
      // Balik gambar (mirror) jika pakai kamera depan
      if (facingMode === 'user') {
      ...
      ctx.fillStyle = "#38bdf8"; // Warna cyan/emerald
      ctx.font = "bold 14px Arial";
      ctx.fillText("STATUS: EXCELLENCE LIVE", 35, canvas.height - 30);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          <div className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-slate-800 w-full max-w-sm rounded-[2rem] overflow-hidden shadow-2xl flex flex-col relative border border-slate-700">
              ...
              <div className="px-5 py-4 flex justify-between items-center bg-slate-800 absolute top-0 w-full z-20 shadow-sm border-b border-slate-700">
                <span className="text-white font-bold tracking-wide text-xs bg-emerald-500/20 text-emerald-400 px-3 py-1.5 rounded-lg border border-emerald-500/30">
                  Form Excellence
                </span>
              ...
              <div className="relative w-full aspect-[3/4] max-h-[60vh] bg-black flex items-center justify-center overflow-hidden mt-16">
                ...
                <div className="absolute bottom-3 left-3 right-3 bg-black/50 backdrop-blur-md p-3 rounded-xl border border-white/20 text-white pointer-events-none">
                  ...
                  <div className="inline-block bg-emerald-500/30 text-emerald-300 px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-wider border border-emerald-500/50">
                    STATUS: EXCELLENCE
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-800 p-5 flex flex-col items-center justify-center">
                <button 
                  ...
                  className="relative flex items-center justify-center w-16 h-16 rounded-full border-[3px] border-slate-300 hover:border-emerald-400 transition-colors duration-300 focus:outline-none"
                >
        ...
        {file && !isSuccess && !isCameraActive && (
          <div className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-md flex flex-col items-center justify-center animate-fade-in p-6">
            <h2 className="text-white text-2xl font-bold mb-6">Preview Penampilan</h2>
            ...
            <div className="flex gap-4 w-full max-w-sm">
              ...
              <button onClick={() => setFile(file)} className="flex-1 py-4 bg-emerald-500 text-white font-bold rounded-2xl hover:bg-emerald-600 transition-colors shadow-lg shadow-emerald-500/30">
                Lanjut Isi Form
              </button>
            </div>
          </div>
                ...
                <div className="pl-0 sm:pl-8">
                  <div className="flex flex-col group cursor-pointer w-full sm:w-1/2 mx-auto" onClick={() => !file && startCamera()}>
                    ...
                    <div className={`relative h-64 flex justify-center items-center px-6 py-6 border-2 border-dashed rounded-2xl transition-all duration-300 overflow-hidden ${file ? 'border-emerald-500 bg-black' : 'border-slate-300 group-hover:border-emerald-400 group-hover:bg-emerald-50/30 group-hover:shadow-[0_4px_20px_rgba(16,185,129,0.05)]'}`}>
                      ...
                      {file ? (
                        <>
                          <img src={file} alt={`Hasil Jepretan`} className="absolute inset-0 w-full h-full object-contain opacity-95 bg-black" />
